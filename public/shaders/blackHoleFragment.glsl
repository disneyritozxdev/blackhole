...
void main()	{
  ...
  if (lensing) {
    // Leapfrog geodesic - gravitational lensing enabled
    ...
    if (distance > 1.0){
      ...
      if (show_stars) {
        ...
        if (star_color.g > 0.0){
          ...
          // reduce star brightness so they don't trigger bloom
          vec3 starColor = temp_to_color(star_temperature) * star_color.g;
          color += vec4(starColor * 0.3, 1.0); // reduce brightness to avoid bloom
        }
      }

      // reduce background brightness so it doesn't trigger bloom
      if (show_milkyway) {
        color += texture2D(bg_texture, tex_coord) * 0.15;
      }
    }
  } else {
    // no lensing - straight ray tracing without gravitational distortion
    // still use geodesic but with zero gravity (straight lines)
    vec3 point = cam_pos;
    vec3 velocity = ray_dir;
    float distance = length(point);
    
    // check if ray hits event horizon (simple sphere check)
    vec3 oc = cam_pos;
    ...
    if (discriminant >= 0.0) {
      ...
      if (t1 > 0.0) {
        hit_horizon = true;
      } else if (t2 > 0.0 && dot(oc, oc) < 1.0) {
        // camera inside event horizon
        hit_horizon = true;
      }
    }
    ...
    if (hit_horizon) {
      ...
      // render accretion disk first (so it can be occluded by black hole)
      if (accretion_disk) {
        // check if ray intersects the disk plane (y=0)
        if (abs(ray_dir.y) > 0.001) {
          float disk_t = -cam_pos.y / ray_dir.y;
          if (disk_t > 0.0) {
            vec3 disk_pos = cam_pos + ray_dir * disk_t;
            float r = length(disk_pos);
            
            // check if disk is behind black hole
            vec3 to_disk = disk_pos;
            float dist_to_disk = length(to_disk);
            float dist_to_horizon = length(cam_pos) - 1.0;
            
            if (dist_to_disk > dist_to_horizon && DISK_IN <= r && r <= DISK_IN + DISK_WIDTH) {
              float phi = atan(disk_pos.x, disk_pos.z);
              phi -= time;
              phi = mod(phi, PI*2.0);
              ...
              if (use_disk_texture) {
                ...
                if (doppler_shift) {
                  float ray_doppler_factor = ray_gamma * (1.0 + dot(ray_dir, -cam_vel));
                  disk_temperature /= ray_doppler_factor;
                }
                vec3 disk_color = temp_to_color(disk_temperature);
                float disk_alpha = clamp(dot(disk_color,disk_color)/3.0,0.0,1.0);
                color += vec4(disk_color, 1.0)*disk_alpha;
      ...
      // render background without distortion (reduced brightness to avoid bloom)
      vec2 tex_coord = to_spherical(ray_dir * ROT_Z(45.0 * DEG_TO_RAD));
      if (show_stars) {
        vec4 star_color = texture2D(star_texture, tex_coord);
        if (star_color.g > 0.0){
